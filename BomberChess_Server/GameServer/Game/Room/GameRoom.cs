...
namespace GameServer
{
    public class GameRoom
    {
        ...
        public void ActiveBomb(Bomb bomb)
        {
            if (bomb == null) return;

            List<Vector3> splashCells = bomb.GetSplashCellPos();
            
            // 폭발 범위 내의 플레이어들에게 데미지 판정
            foreach (Player player in _players.Values)
            {
                Vector3 playerPos = new Vector3(player.Info.PosInfo.PosX, player.Info.PosInfo.PosY, player.Info.PosInfo.PosZ);
                if (splashCells.Contains(playerPos))
                {
                    Console.WriteLine($"{player.Info.ObjectId} Hit by Bomb");
                    // TODO: 데미지 처리 로직 추가
                    //player.Ondamaged(bomb.BombDamage);
                }
            }
            Console.WriteLine($"Bomb Actived {bomb.Id}");
            // 폭탄 제거
            LeaveGame(bomb.Id);
        }
